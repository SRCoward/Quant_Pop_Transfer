...
int qasm_to_quest(int n){
  ...
  initZeroState(qubits); // may need to include some way of initialising the state here
  ...
  while (!inFile.eof()){
    ...
    if (!op.compare("cx")){ // IF GATE IS CX
      // NOTE THIS IS VERY FLIMSY!!!!!
      //std::cout<<data<<std::endl;
      char control = data[5];
      char target = data[10];
      ...
      //std::cout<<target<<int_control<<std::endl;
      controlledNot(qubits,int_control,int_target);
    } else if (!op.compare("u1")){ // IF GATE IS U1
    ...
    } else if (!op.compare("u2")){ //IF THE GATE IS U2
